...
// Search
  ...
  const buildFuzzyKeywords = searchText => {
    const baseKeywords = searchText.split(/[-\s]+/).filter(Boolean)
    const keywordSet = new Set(baseKeywords)
    if (baseKeywords.length === 1) {
      const word = baseKeywords[0]
      const hasCjk = /[\u4e00-\u9fff]/.test(word)
      if (hasCjk && word.length >= 2) {
        for (let i = 0; i < word.length - 1; i++) {
          keywordSet.add(word.slice(i, i + 2))
        }
      }
    }
  ...
  const inputEventFunction = () => {
    ...
    const keywords = buildFuzzyKeywords(searchText)
    let resultItems = []
